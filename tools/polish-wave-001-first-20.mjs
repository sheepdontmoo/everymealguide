...
const targets = manifest.slice(0, 20);
const updated = [];
const skipped = [];
...
fs.writeFileSync(path.join(reportsDir, 'wave-001-first-20-polish.json'), JSON.stringify(report, null, 2), 'utf8');
fs.writeFileSync(path.join(reportsDir, 'wave-001-first-20-polish.md'), `# Wave 001 First 20 Page Polish\n\nGenerated: ${report.generatedAt}\n\n- Target pages: ${report.targetCount}\n- Pages updated: ${report.pagesUpdated}\n- Skipped: ${report.skipped.length}\n\n## Updated pages\n\n${updated.map(row => `- ${row.target_path} — ${row.title}`).join('\n')}\n`, 'utf8');
